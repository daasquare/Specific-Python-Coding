...
# Read the Excel file
excel_file = pd.ExcelFile('/Users/akin.o.akinjogbin/Desktop/July 2023 Tine Testing Raw data/Tine Engagement /Proccessed Files/Versions Runs/Version 1-2.log.xlsx')
...
# Function to create virtual windows, get the index of the Max Peak within each window,
# and return the X and Y values at the end of each window
def get_max_peak_in_windows(x_data, y_data, window_size, num_windows, start_x, end_x):
    max_peak_idx = []
    end_x_values = []
    end_y_values = []
    ...
    # Find the index of the first element in x_data that is greater than or equal to end_x
    end_idx = np.argmax(x_data >= end_x)

    step = (end_idx - start_idx) // num_windows
...
for i, sheet_name in enumerate(excel_file.sheet_names):
    ...
    # Specify the start_x_for_windows and end_x_for_windows values
    start_x_for_windows = 19  # Windows will start after X=20
    end_x_for_windows = 40   # Modify this value as needed

    # Create virtual windows and get index of Max Peak, along with the X and Y values at the end of each window
    window_size = len(x) // num_windows  # Adjust the window size based on the desired number of windows
    max_peak_idx, end_x_values, end_y_values = get_max_peak_in_windows(x, filtered_y, window_size, num_windows, start_x_for_windows, end_x_for_windows)
